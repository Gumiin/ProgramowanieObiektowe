...
class Program
{
    ...
    {
        return delegate (string s, int i)
        {
            var x="";
            for(int j = 0; j < i; j++)
            {
    ...
    {
        return (c) => c.Length == 9 && c.GetType() == typeof(string);
    }
    ...
    {
            return LoadPeople(data, str =>
            {
                string part1 = str.Split(' ')[0];
                string part2 = str.Split(' ')[1];
                return part1.Length == 9 && int.Parse(part2) >= 0;
            });
    }
